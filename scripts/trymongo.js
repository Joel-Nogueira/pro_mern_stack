...
function testWithCallbacks(){
	MongoClient.connect('mongodb://localhost/playground', function(err, db){
		db.collection('employees').insertOne({id: 1, nome: 'A. Callback'},
		function(err, result){
			console.log("Result of insert: ", result.insertedId);
			db.collection('employees').find({id: 1}).toArray(function(err, docs){
				console.log("Result of kind: ", docs);
				db.close();
			});
		});
	});
...
function testWithPromises(){
	...
	MongoClient.connect('mongodb://localhost/playground').then(connection => {
		db = connection;
		return db.collection('employees')....insertOne({id: 1, nome: 'B. Promises'});
	}).then(result => {
		console.log("Result of insert: ", result.insertedId);
...
function testWithGenerator(){
	...
	co(function*(){
		...
		const result = yield db.collection('employees')...insertOne({id: 1, nome: 'C. Generator'});
		console.log("Result of insert: ", result.insertedId);
...
function testWithAsync(){
	...
	async.waterfall([
		...
		(connection, next) => {
			db = connection;
			db.collection('employees').insertOne({id: 1, nome: 'D. Async'}, next);
		},
		(insertResult, next) => {
		...
		(docs, next) => {
			console.log("Reslt of kind: ", docs);
			db.close();
			next(null, 'All done');
		}
	], (err, result) => {
